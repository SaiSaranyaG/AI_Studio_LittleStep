...
export const AirEnvironmentView: React.FC = () => {
  ...
  const handleEstablishBaseline = async () => {
    await updateBaseline({
      ...
      indoorTemp: {
        value: Number(indoorTempInput) || 24,
        unit: '°C',
        sourceType: 'USER_PROVIDED',
        sourceLabel: 'Home Sensor / Thermometer',
        confidence: 0.95,
      },
      indoorHumidity: {
        value: Number(indoorHumidityInput) || 50,
        unit: '%',
        sourceType: 'USER_PROVIDED',
        sourceLabel: 'Calibrated Hygrometer',
        confidence: 0.95,
      },
      isUserVerified: true,
    });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {isCalibrateOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-emerald-950 border border-emerald-700/80 rounded-2xl max-w-lg w-full p-6 space-y-5 animate-scaleUp">
            <div className="flex items-center justify-between border-b border-emerald-800/50 pb-3">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Activity className="w-5 h-5 text-emerald-400" />
                <span>Establish Environmental Baseline</span>
              </h3>
              <button onClick={() => setIsCalibrateOpen(false)} className="text-emerald-400 hover:text-white text-sm">
                ✕
              </button>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button onClick={() => setIsCalibrateOpen(false)} className="px-4 py-2 text-xs text-emerald-300 hover:text-white">
                Cancel
              </button>
              <button
